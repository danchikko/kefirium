...
const Famous = () => {
	...
	useEffect(() => {
		fetch('http://localhost:3000/Categories')
		 ...
		 .then((data) => setState(data.slice(1,7)))
	}, [])
	return (
		<Container>
			...
			{state.map((el) => {
				return (
					<Card key={el.id}>
						...
						<Content>
							<Title size='16px'>{el.collectioName}</Title>
							<Description size='14px'>{el.collectionDescription}</Description>
							<Flex>
								<UserImg
								...
								<div>
									...
									<Title size='14px'>{el.nickname}</Title>
								</div>
							</Flex>
						</Content>
					</Card>
