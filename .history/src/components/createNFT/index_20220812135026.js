...
const CreateFormNFT = () => {
	...
	const { NFTImage, NFTName, description, reference, content, price } =
	...
	useEffect(() => {
		...
		let nicknames = nicknameFilter.filter((el) => el.email === user.email)
		return nicknames.map((el) => setNickname(el.nickname))
	}, [])
	useEffect(() => {
		let nick = nicknames.map((el) => setNickname(el.nickname))
