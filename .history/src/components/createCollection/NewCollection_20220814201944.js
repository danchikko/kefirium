...
import { createCollectionBanner, createCollectionCategories, createCollectionDescription, createCollectionImage, createCollectionName } from '../../store/postsSlice'
...
const NewCollection = () => {
	...
	return (
		<ModalContent onClick={(e) => e.stopPropagation()}>
				<Title color='#000000' size='40px' margin="30px 0" weight="700">
					Создание коллекции
				</Title>
			...
			<SpecialText>Рекомендуемые размеры для изображения</SpecialText>
			<SpecialText>
			...
			<SpecialText>
				Рекомендуемые размеры для баннера 1920х250 пикселей
			</SpecialText>
			<input
				style={{ display: 'none' }}
				id='banner'
			...
			<InputName
				type='text'
				onChange={(e) => dispatch(createCollectionName(e.target.value))}
			/>
			<Text color='#000000' size='14px' weight='700'>
				Категория*
			</Text>
			...
			<CategoriesSelect
				onChange={(e) =>
					dispatch(createCollectionCategories(e.target.value))
				}
			>
				...
				<option value='collectibles'>Коллекционные предметы</option>
				<option value='virtualworlds'>Виртуальные миры</option>
			</CategoriesSelect>
			<Text color='#000000' size='14px' weight='700'>
				Описание
			</Text>
			<Description
				onChange={(e) =>
					dispatch(createCollectionDescription(e.target.value))
				}
				placeholder='Введите описание'
			/>
			<Flex align='center' width='100%' justify='center' margin='40px 0'>
				<SubmitButton
