...
import { modalRegister } from '../../store/optionsSlice'
...
const Header = () => {
	const { registerModal, signUpModal, signInModal } = useSelector(
		(state) => state.options,
	)
	const { isAuth, registerModal, signUpModal, sign } = useSelector((state) => state.register)
			...
			<HeaderWrapper>
				...
				{isAuth ? (
					<OnRegister
						src={avatar}
						alt='avatar'
						onClick={() => dispatch(modalRegister())}
					/>
				) : (
					<NoRegister onClick={() => dispatch(modalRegister())} />
				)}
				{isAuth ? (
					registerModal ? (
						<CompletionRegister />
