...
import { modalSignUp, onRegister } from '../../store/registerSlice'
...
const FormPhysic = () => {
	...
	const handleSubmit = async (e) => {
		...
		try{
			...
			dispatch( onRegister(true), dispatch(modalSignUp()))
		}catch(e) {
			setError(e.message)
	...
	return (
		<Form onSubmit={handleSubmit}>
			...
			<Inputs onChange={(e) => setEmail(e.target.value)} placeholder='Введите свой Email' type='email' />
			<Text color='#000000' weight='700'>
				Пароль*
			</Text>
			<Inputs onChange={(e) => setPassword(e.target.value)} placeholder='Пароль' type='password' />
