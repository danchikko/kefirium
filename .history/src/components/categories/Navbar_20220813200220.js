...
import { allClick, artClick, collectiblesClick, musicClick, photoClick, sportClick, virtualworldsClick } from '../../store/optionsSlice'

const Navbar = () => {
	const {all, art, sport, music, photo, collectibles, virtualworlds} = useSelector((state) => state.options)
	...
	return (
		<NavDiv>
			...
			{collectibles ? (
				...
				<NavButtonFalse onClick={() => dispatch(collectiblesClick())}>
            ...
            {virtualworlds ? (
				...
				<NavButtonFalse onClick={() => dispatch(virtualworldsClick())}>
					Виртуальные миры
				</NavButtonFalse>
			)}
		</NavDiv>
